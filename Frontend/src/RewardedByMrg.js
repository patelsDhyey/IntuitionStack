...
function RewardedByMrg() {
  ...
  useEffect(() => {
    fetch(`http://localhost:3001/RewardedByMrg/${sessionStorage.getItem("sessionid")}`).then((response) => response.json()).then((data) => {
      console.log(data)
  ...
  if (!(sessionStorage.getItem("sessionkey") === null)) {
    ...
    return (
      <>
        ...
        <div className="rewardedbymrg">
          <div className="rewardedbymrg-form">
            {/* <Link className="home-header-back" to="/Employee/Dashboard">&#8592;</Link> */}
            <div className="home-header-back" onClick={FireBack}>&#8592;</div>
            <div className='rewardedbymrg-title'>Rewarded Intuitions By You</div>
            <div className="rewardedbymrg-displaydata">
              {FetchedIntuitions.map(e => {
                return (
                  <div className="rewardedbymrg-content" key={e.intid}>
                    <label className='rewardedbymrg-content-brief' >  {e.intBrief}  </label>
                    <label className='rewardedbymrg-content-desc'  >{e.intDesc}  </label>
                    <label className='rewardedbymrg-content-status' >{e.StatusName}  </label>
                    <label className='rewardedbymrg-content-posted' >Posted on: &nbsp;{e.intDate},&nbsp;{e.intTime}  </label>
                    <label className='rewardedbymrg-content-tags' id={e.intid}>{e.intTags.split(" ").map((el, key) => {
                      return <label className='rewardedbymrg-content-tags-tag' key={key}>{el}</label>
                    })}  </label>
                    <div className="rewardedbymrg-menu-feedback">
