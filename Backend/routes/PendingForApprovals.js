...
router.get('/:id', function (req, res, next) {
  ...
  // var fetchStatus = `SELECT * FROM Intuitions WHERE intByEmpId = ${req.params.id} AND intStatusId = (SELECT stsId FROM IntuitionStatus WHERE StatusName = "New Post");`
  var fetchStatus = `SELECT * FROM Intuitions INNER JOIN IntuitionStatus ON stsId = intStatusId WHERE intByEmpId = ${req.params.id} AND intStatusId = (SELECT stsId FROM IntuitionStatus WHERE StatusName = "New Post");`
